...
function countdown() {
  ...
  const days = Math.floor(
    (delta % (1000 * 60 * 60 * 24 * 30)) / (1000 * 60 * 60 * 24)
  );
  ...
  const deltaArray = [];
  if (months > 0 && Object.keys(deltaArray).length <= 1)
    deltaArray.push({ label: "months", value: months });
  if (days > 0 && Object.keys(deltaArray).length <= 1)
    deltaArray.push({ label: "days", value: days });
  if (hours > 0 && Object.keys(deltaArray).length <= 1)
    deltaArray.push({ label: "hours", value: hours });
  if (minutes > 0 && Object.keys(deltaArray).length <= 1)
    deltaArray.push({ label: "minutes", value: minutes });
  if (seconds > 0 && Object.keys(deltaArray).length <= 1)
    deltaArray.push({ label: "seconds", value: seconds });

  const elementsArray = [
    { label: monthsLabelEl, value: monthEl },
    { label: daysLabelEl, value: daysEl },
  ];

  for (let i = 0; i < deltaArray.length; i++) {
    const { label, value } = deltaArray[i];
    elementsArray[i].label.textContent = label;
    elementsArray[i].value.textContent = value;
  }
}
